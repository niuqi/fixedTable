		...
		*/
			...
			FixedTable.prototype = {
				...
				styleObj:{
					...
					fixedHeadWrapper:{'position':'absolute','top':0,'left':0,'overflow':'hidden'},
					fixedLeftWrapper:{'position':'absolute','top':0,'left':0,'overflow':'hidden'},
					fixedConnectWrapper:{'position':'absolute','top':0,'left':0,'overflow':'hidden'}
				},
				creatFixedHeadWrapper:function(){
					...
					_this.opt.fixedHeadWrapper.css(_this.styleObj.fixedHeadWrapper);
				...
				creatFixedLeftWrapper:function(){
					...
					_this.opt.fixedLeftWrapper = $('<div id="'+_this.opt.id+'_fixedLeftWrapper"></div>');
					_this.opt.fixedLeftWrapper.appendTo(_this.opt.wrapper);
					_this.opt.fixedLeftWrapper.css(_this.styleObj.fixedLeftWrapper);
					/*判断是否固定表头*/
					if(_this.opt.fixedThead){
					...
					tbodyHtml += '</tbody>';
					/*填充入fixedLeftWrapper*/
					_this.opt.fixedLeftWrapper.html('<table style="position:relative;table-layout:fixed">'+theadHtml+tbodyHtml+'</table>');
					/*计算宽度*/
					...
					/*判断是否同时固定表头和列*/
					if(_this.opt.fixedThead && _this.opt.fixedLeft){
						_this.opt.fixedLeftWidth = width;
					};
				},
				creatConnectWrapper:function(){
					var _this = this,
						html = _this.opt.fixedLeftWrapper.find('thead').html();
					_this.opt.fixedConnectWrapper = $('<div id="'+_this.opt.id+'_fixedConnectWrapper"><table style="position:relative;table-layout:fixed;width:'+_this.opt.fixedLeftWidth+'px;"><thead>'+html+'</thead></table></div>');
					_this.opt.fixedConnectWrapper.appendTo(_this.opt.wrapper);
					_this.opt.fixedConnectWrapper.css(_this.styleObj.fixedConnectWrapper);
				},
